...
func TestAccRedfishDirectoryServiceAuthProviderCertificateRSABasic(t *testing.T) {
	valid_ds_cert := os.Getenv("VALID_DS_CERT")
	valid_ds_cert_update := os.Getenv("VALID_DS_CERT_UPDATE")
	invalid_ds_cert := os.Getenv("INVALID_DS_CERT")
	terraformDSAuthProviderCertificateResourceName := "redfish_directory_service_auth_provider_certificate.ds_auth_certificate"
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				// create certificate import resource
				Config: testAccRedfishDirectoryServiceAuthProviderCertificateConfig(creds, valid_ds_cert, "RSA_CA_CERT"),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr(terraformDSAuthProviderCertificateResourceName, "certificate_type", "RSA_CA_CERT"),
				),
			},

			{
				// update certificate import resource
				Config: testAccRedfishDirectoryServiceAuthProviderCertificateConfig(creds, valid_ds_cert_update, "RSA_CA_CERT"),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr(terraformDSAuthProviderCertificateResourceName, "certificate_type", "RSA_CA_CERT"),
				),
			},
			{
				// update certificate import resource with invalid cert
				Config:      testAccRedfishDirectoryServiceAuthProviderCertificateConfig(creds, invalid_ds_cert, "RSA_CA_CERT"),
				ExpectError: regexp.MustCompile("There was an error while creating/updating Certificate resource"),
			},
		},
	})
