...
func (r *RedfishDirectoryServiceAuthProviderResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
...
func (r *RedfishDirectoryServiceAuthProviderResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	type creds struct {
...
func (r *RedfishDirectoryServiceAuthProviderResource) updateRedfishDirectoryServiceAuth(ctx context.Context, service *gofish.Service, plan, state *models.DirectoryServiceAuthProviderResource,
) diag.Diagnostics {
...
func updateActiveDirectory(ctx context.Context, serviceURI string, service *gofish.Service, plan *models.DirectoryServiceAuthProviderResource) (diags diag.Diagnostics) {
	...
	if dcLookupDomainCheck, diags := isValidDCLookupDomainConfig(ctx, "ActiveDirectory", "DCLookupEnable", plan); diags.HasError() || !dcLookupDomainCheck {
		return diags
	}
	...
	if err != nil {
		diags.AddError("There was an error while creating/ updating Active Directory", "There was an error while creating/ updating Active Directory "+err.Error())
		return diags
	}
	if response != nil {
		body, err := io.ReadAll(response.Body)
		if err != nil {
			diags.AddError("error reading response body", "error coming ********"+string(body))
	...
	// resp.Diagnostics.Append(diags...)
	if diags.HasError() {
		//diags.AddError("Idrac update fail", "Idrac update fail due to invalid Active Directory configuration")
		return diags
	}
