...
func newActiveDirectoryChanged(ctx context.Context, plan, state *models.DirectoryServiceAuthProviderResource) bool {
	...
	/*planServiceAddress := directoryPlan.ServiceAddresses
	stateServiceAddress := directoryState.ServiceAddresses
	changed := false
	for _, planTarget := range planServiceAddress {
		changed = true
		for _, stateTarget := range stateServiceAddress {
...
func newLDAPChanged(ctx context.Context, plan, state *models.DirectoryServiceAuthProviderResource) bool {
	...
	if ldapSearchSettingsPlan.GroupNameAttribute.ValueString() != "" && ldapSearchSettingsPlan.GroupNameAttribute.ValueString() != ldapSearchSettingsState.GroupNameAttribute.ValueString() {
		return true
	}
	if ldapSearchSettingsPlan.UsernameAttribute.ValueString() != "" && ldapSearchSettingsPlan.UsernameAttribute.ValueString() != ldapSearchSettingsState.UsernameAttribute.ValueString() {
		return true
	}
...
func checkAttributeskeyPresent(attributes types.Map, prefix string, suffix string) bool {
	for k, _ := range attributes.Elements() {
...
func isSSOEnabledWithValidFile(ctx context.Context, prefix string, suffix string, attrsState *models.DirectoryServiceAuthProviderResource) (bool, diag.Diagnostics) {
	...
	if check {
		...
		if value == enabled {
			...
			if activeDirectoryPlan.Authentication.IsNull() || activeDirectoryPlan.Authentication.IsUnknown() || authenticationPlan.KerberosKeytab.IsNull() || authenticationPlan.KerberosKeytab.IsUnknown() {
				diags.AddError("Please provide valid kerberos key tab file when SSO is enabled", "Please provide valid kerberos key tab file when SSO is enabled")
				return false, diags
			}
	...
	if check {
		...
		if value == "Extended Schema" {
			if !iDRAcName || !iDRAcDomain {
				diags.AddError("Please provide valid config for RacName and RacDomain", "RacName and RacDomain must be configured for Extended Schema")
				return false, diags
			}
			...
			if iDRAcNameValue == "" || iDRAcDomainValue == "" {
				diags.AddError("RacName and RacDomain can not be null for Extended Schema", "Please provide valid configuration for RacName and RacDomain")
				return false, diags
			}
			if gcLookupEnable || gc1 || gc2 || gc3 {
				diags.AddError("GCLookupEnable, GlobalCatalog1, GlobalCatalog2, GlobalCatalog3 can not be configured for Extended Schema", "GCLookupEnable, GlobalCatalog1, GlobalCatalog2, GlobalCatalog3 can not be configured for Extended Schema")
				return false, diags
			}

			if !activeDirectoryPlan.Directory.IsNull() && !activeDirectoryPlan.Directory.IsUnknown() {
				if len(remoteRoleMapping) != 0 {
					diags.AddError("RemoteRoleMapping can not be configured for Extended Schema", "RemoteRoleMapping can not be configured for Extended Schema")
					return false, diags
		...
		if value == "Standard Schema" {
			if iDRAcName || iDRAcDomain {
				diags.AddError("RacName and RacDomain can not be configured for Standard Schema", "RacName and RacDomain can not be configured for Standard Schema")
				return false, diags
			}
			if !gcLookupEnable {
			...
			if gcLookupEnableValue == "Enabled" {
				...
				if !gcRootDomain {
					diags.AddError("GCRootDomain must be configured for Enabled GCLookupEnable", "GCRootDomain must be configured for Enabled GCLookupEnable")
					return false, diags
				}
				gcRootDomainValue := getkAttributeskeyValue(attributes, "ActiveDirectory", "GCRootDomain")

				if gcRootDomainValue == "" {
					diags.AddError("GCRootDomain must be configured for Enabled GCLookupEnable", "GCRootDomain must be configured for Enabled GCLookupEnable")
					return false, diags
				}

				if gc1 || gc2 || gc3 {
					diags.AddError("GlobalCatalog can not be configured for Enabled GCLookupEnable", " GlobalCatalog can not be configured for Enabled GCLookupEnable")
					return false, diags
				}
				...
				if gc1Value == "" && gc2Value == "" && gc3Value == "" {
					diags.AddError("Invalid GlobalCatalog configuration for Standard Schema", "Atleast any one from GlobalCatalog1, GlobalCatalog2, GlobalCatalog3 must be configured for Disabled GCLookupEnable")
					return false, diags
				}

				gcRootDomain := checkAttributeskeyPresent(attributes, "ActiveDirectory", "GCRootDomain")
				if gcRootDomain {
					diags.AddError("GCRootDomain can not be configured for Disabled GCLookupEnable", "GCRootDomain can not be configured for Disabled GCLookupEnable")
					return false, diags
				}
			} else {
...
func isValidDCLookupDomainConfig(ctx context.Context, prefix string, suffix string, attrsState *models.DirectoryServiceAuthProviderResource) (bool, diag.Diagnostics) {
	...
	if check {
		...
		// value := getkAttributeskeyValue(attributes, prefix, suffix)
		// dcLookupEnable := checkAttributeskeyPresent(attributes, "ActiveDirectory", "DCLookupEnable")
		objectAsOptions := basetypes.ObjectAsOptions{UnhandledNullAsEmpty: true, UnhandledUnknownAsEmpty: true}
		var activeDirectoryPlan models.ActiveDirectoryResource
		if !attrsState.ActiveDirectoryResource.IsNull() && !attrsState.ActiveDirectoryResource.IsUnknown() {
			if diags := attrsState.ActiveDirectoryResource.As(ctx, &activeDirectoryPlan, objectAsOptions); diags.HasError() {
		...
		if dcLookupEnableValue == "Disabled" {
			/*if activeDirectoryPlan.Directory.IsNull() || activeDirectoryPlan.Directory.IsUnknown() {
				diags.AddError("Invalid configuration for DCLookUp", "Service address must be configured for Disabled DCLookUp")
				return false, diags
			}*/
			...
			if len(serviceAddressList) == 0 {
				...
				//if len(directoryPlan.ServiceAddresses) == 0 || (directoryPlan.ServiceAddresses[0].ValueString() == "" && directoryPlan.ServiceAddresses[1].ValueString() == "" && directoryPlan.ServiceAddresses[2].ValueString() == "") {
				diags.AddError("ServiceAddresses is not Configured for Disabled DCLookUp", "Atleast one Service address must be configured for Disabled DCLookUp")
				return false, diags
			}
			if userDomain {
				diags.AddError("DCLookupByUserDomain can not be Configured for Disabled DCLookUp", "DCLookupByUserDomain can not be Configured for Disabled DCLookUp")
				return false, diags
			}
			if specifyDomain {
				diags.AddError("DCLookupDomainName can not be configured for Disabled DCLookUp", "DCLookupDomainName can not be configured for Disabled DCLookUp")
				return false, diags
			}
			...
			if !userDomain {
				diags.AddError("DCLookupByUserDomain must be configured for Enabled DCLookUp", "DCLookupByUserDomain must be configured for Enabled DCLookUp")
				return false, diags
			}
			...
			if userDomainValue == "Disabled" {
				//specifyDomain := checkAttributeskeyPresent(attributes, "ActiveDirectory", "DCLookupDomainName")
				if !specifyDomain {
					diags.AddError("DCLookupDomainName must be configured for Disabled DCLookupByUserDomain", "DCLookupDomainName must be configured for Disabled DCLookupByUserDomain")
					return false, diags
				}
				...
				specifyDomain := checkAttributeskeyPresent(attributes, "ActiveDirectory", "DCLookupDomainName")
				if specifyDomain {
					diags.AddError("DCLookupDomainName can not be configured for Enabled DCLookupByUserDomain", "DCLookupDomainName can not be configured for Enabled DCLookupByUserDomain")
					return false, diags
				}
			}
			return true, diags
